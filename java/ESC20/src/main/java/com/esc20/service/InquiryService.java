...
public class InquiryService {
	...
	public List<Deduction> retrieveDeductions(String employeeNumber, Frequency frequency)
	{
		if(frequency != null)
		{
			return deductionsDao.getDeductions(employeeNumber, frequency);
		}
		else
		{
	...
	public Earnings getTYDEarnings(String employeeNumber, List<PayDate> payDates, PayDate payDate) {
		...
		for(int i=0; i<earningsOther.size();i++) {
			earningsOther.get(i).setTydAmt(earningsOther.get(i).getAmt()==null?BigDecimal.valueOf(0):earningsOther.get(i).getAmt());
			earningsOther.get(i).setTydContrib(earningsOther.get(i).getContrib()==null?BigDecimal.valueOf(0):earningsOther.get(i).getContrib());
		...
		for(int i=0; i<ytdEarnings.getOther().size();i++) {
			ytdEarnings.setEarningsOtherTydTotal(ytdEarnings.getEarningsOtherTydTotal().add(ytdEarnings.getOther().get(i).getTydAmt()));
		}
		return ytdEarnings;
	}
	...
	private Earnings sumUpEarnings(Earnings ytdEarnings, Earnings fetchedEarnings) {
		...
		for(int i=0; i<earningsOther.size();i++) {
			for(int j=0; j<fetchedOther.size();j++) {
				if(earningsOther.get(i).getCode().equals(fetchedOther.get(j).getCode())) {
					earningsOther.get(i).setTydAmt(earningsOther.get(i).getTydAmt().add(fetchedOther.get(j).getAmt()==null?BigDecimal.valueOf(0):fetchedOther.get(j).getAmt()));
					earningsOther.get(i).setTydContrib(earningsOther.get(i).getTydContrib().add(fetchedOther.get(j).getContrib()==null?BigDecimal.valueOf(0):fetchedOther.get(j).getContrib()));
				}
			}
		...
		ytdEarnings.setEarningsOtherTotal(ytdEarnings.getEarningsOtherTotal().add(fetchedEarnings.getEarningsOtherTotal()));
		ytdEarnings.setEarningsOtherContribTotal(ytdEarnings.getEarningsOtherContribTotal().add(fetchedEarnings.getEarningsOtherContribTotal()));
		ytdEarnings.setEarningsJobTotal(ytdEarnings.getEarningsJobTotal().add(fetchedEarnings.getEarningsJobTotal()));
		ytdEarnings.setEarningsSupplementalTotal(ytdEarnings.getEarningsSupplementalTotal().add(fetchedEarnings.getEarningsSupplementalTotal()));
		ytdEarnings.setEarningsNonTrsTaxTotal(ytdEarnings.getEarningsNonTrsTaxTotal().add(fetchedEarnings.getEarningsNonTrsTaxTotal()));
		ytdEarnings.setEarningsNonTrsNonTaxTotal(ytdEarnings.getEarningsNonTrsNonTaxTotal().add(fetchedEarnings.getEarningsNonTrsNonTaxTotal()));
		ytdEarnings.setEarningsBankTotal(ytdEarnings.getEarningsBankTotal().add(fetchedEarnings.getEarningsBankTotal()));
		ytdEarnings.setEarningsOvertimeTotal(ytdEarnings.getEarningsOvertimeTotal().add(fetchedEarnings.getEarningsOvertimeTotal()));
		return ytdEarnings;
	}
	...
	public List<PayDate> getAvailablePayDates(String employeeNumber, Integer restrictPayDate) {
		...
		for(int i = 0; i < payDates.size(); i++) {
			...
			if(prefix.equals("CYTD")||prefix.equals("AVAR")||prefix.equals("SYTD")||prefix.equals("SADJ")||prefix.equals("ADJU")||prefix.equals("CTRS"))			{
				payDates.remove(i);
				i--;
				continue;
			}
...
//		}
	...
	public W2Print getW2Print(BhrW2 w2Info, BhrEmpDemo user, District employer) {
		...
		print.setEmpname(user.getNameF() + " " + middleName + " " + user.getNameL());   //jf20150113 Display description instead of code fix
