...
public class IndexService {
	...
	public void updatePayInfo(BhrEmpDemo demo, BhrEmpPay pay, Character payFreq, Character maritalStatTaxNew,
			Integer nbrTaxExemptsNew, String w4FileStatNew, String w4MultiJobNew, Integer w4NbrChldrnNew,
			Integer w4NbrOthrDepNew, Double w4OthrIncAmtNew, Double w4OthrDedAmtNew, Double w4OthrExmptAmtNew) {
		payDao.updatePayInfo(demo, pay, payFreq, maritalStatTaxNew, nbrTaxExemptsNew, w4FileStatNew, w4MultiJobNew,
				w4NbrChldrnNew, w4NbrOthrDepNew, w4OthrIncAmtNew, w4OthrDedAmtNew, w4OthrExmptAmtNew);
	}
	...
	/*
	 * public Boolean isSupervisor(String empNbr) { return
	 * userDao.isSupervisor(empNbr); }
	 */
	...
	public void passwordChangeSendEmailConfirmation(String userName, String userFirstName, String userLastName,
			String userHomeEmail, String userWorkEmail) {
		...
		if (toEmail != null && toEmail.trim().length() > 0) {
			...
			logger.info(
					"Self Service Change Password: Unable to send an email confirmation.  No email address is avaiable for user "
							+ userName + ".");
		}

	}

	public void personDataChangeSendEmailConfirmation(BhrEmpDemo oldValue, BhrEmpDemo newValue,
			DemoInfoFields demoInfoChanges, DemoInfoFields docRequiredFields) {
		String subject = "A MESSAGE FROM SELF SERVICE";
		StringBuilder messageContents = new StringBuilder();
		...
		userLastName = userLastName == null ? "" : userLastName.trim();
		/*
		 * employeeMessageContents.append((userFirstName == null ||
		 * userFirstName.trim().equals("")) ? "" : userFirstName.trim());
		 * employeeMessageContents.append((userMiddleName == null ||
		 * userMiddleName.trim().equals("")) ? "" : " " + userMiddleName.trim());
		 * employeeMessageContents.append((userLastName == null ||
		 * userLastName.trim().equals("")) ? "" : " " + userLastName.trim());
		 * employeeMessageContents.append((userGeneration == null ||
		 * userGeneration.trim().equals("")) ? "" : " " + userGeneration.trim());
		 */
		...
		if (toEmail != null && toEmail.trim().length() > 0) {
			...
			logger.info(
					"Self Service Change Demo Info: Unable to send an email confirmation.  No email address is avaiable");
		}
		...
		approverEmailMessage
				.append((userFirstName == null || userFirstName.trim().equals("")) ? "" : userFirstName.trim());
		// approverEmailMessage.append((userMiddleName == null ||
		// userMiddleName.trim().equals("")) ? "" : " " + userMiddleName.trim());
		approverEmailMessage
				.append((userLastName == null || userLastName.trim().equals("")) ? "" : " " + userLastName.trim());
		// approverEmailMessage.append((userGeneration == null ||
		// userGeneration.trim().equals("")) ? "" : " " + userGeneration.trim());
		...
		for (String approverNumber : approversToEmail) {
			try {
				...
				appMessageContents.append(approver.getNameF() + " " + approver.getNameL() + ",<br/><br/>"
						+ approverEmailMessage.toString());
				String toApproverEmail = (approverWorkEmail == null || approverWorkEmail.equals("")) ? approverHomeEmail
						: approverWorkEmail;
				if (toNewEmail != null && toNewEmail.trim().length() > 0) {
				...
				logger.info("Self Service Change Demo Info: An exception has occured with mailing the approver "
						+ ex.toString());
			}
		}
	...
	public void payrollDataChangeSendEmailConfirmation(BhrEmpDemo userDemo, String payFreq, boolean payrollSame,
			boolean accountSame, PayInfoChanges currentPayInfoChanges, W4Info w4Info, Boolean autoApproveBank,
			List<BankChanges> currentAccountInfoChanges, PayrollFields docRequired) {
		boolean hasDocChanges = false;
		boolean hasApprovChanges = false;
		...
		if (currentPayInfoChanges.getOtherIncomeAmtChanged() && autoApprove && !payrollSame) {
			contents.append(
					"Other Income:\t\t" + NumberUtil.getDoubleString(w4Info.getW4OthrIncAmt().doubleValue()) + "<br/>");
			hasApprovChanges = true;
			hasPayInfoChanges = true;
		}
		if (currentPayInfoChanges.getOtherDeductAmtChanged() && autoApprove && !payrollSame) {
			contents.append(
					"Deductions: \t\t" + NumberUtil.getDoubleString(w4Info.getW4OthrDedAmt().doubleValue()) + "<br/>");
			hasApprovChanges = true;
			hasPayInfoChanges = true;
		}
		if (currentPayInfoChanges.getOtherExemptAmtChanged() && autoApprove && !payrollSame) {
			contents.append("Other Exemptions:\t\t"
					+ NumberUtil.getDoubleString(w4Info.getW4OthrExmptAmt().doubleValue()) + "<br/>");
			hasApprovChanges = true;
			hasPayInfoChanges = true;
		}
		...
		for (BankChanges b : currentAccountInfoChanges) {
			...
			if (b.getCodeChanged() && autoApproveBank && !accountSame) {
				bankContents.append("Bank Account Information: \t\t" + b.getBank().getCode().getDescription() + "\t"
						+ b.getBank().getCode().getSubCode() + "<br/>");
				hasApprovChanges = true;
			...
			if (b.getAccountTypeChanged() && autoApproveBank && !accountSame) {
				bankContents
						.append("Bank Account Type:\t\t" + b.getBank().getAccountType().getDisplayLabel() + "<br/>");
				hasApprovChanges = true;
				hasAccountChanges = true;
				tempChanges = true;
			}
			if (b.getDepositAmountChanged() && autoApproveBank && !accountSame) {
				bankContents.append("Bank Account Deposit Amount:\t\t"
						+ b.getBank().getDepositAmount().getDisplayAmount() + "<br/>");
				hasApprovChanges = true;
				hasAccountChanges = true;
				tempChanges = true;
			}
		...
		for (BankChanges b : currentAccountInfoChanges) {
			...
			if (b.getAccountTypeChanged() && !accountSame) {
				...
				if (docRequired.getAccountType()) {
					bankContents.append(
							"Bank Account Type:\t\t" + b.getBank().getAccountType().getDisplayLabel() + "<br/>");
					hasDocChanges = true;
				}
			}
			if (b.getDepositAmountChanged() && !accountSame) {
				...
				if (docRequired.getDepositAmount()) {
					if (newprim) {
						bankContents.append("Bank Account Deposit Amount:\t\t"
								+ b.getBank().getDepositAmount().getDisplayAmount() + " (New Primary Account)<br/>");
					} else {
						bankContents.append("Bank Account Deposit Amount:\t\t"
								+ b.getBank().getDepositAmount().getDisplayAmount() + "<br/>");
					}
			...
			if (!autoApprove) {
				...
				if (toApproverEmail != null && toApproverEmail.trim().length() > 0) {
					...
					try {
						...
						logger.info(
								"Self Service Change Payroll Pay Info: An exception has occured with mailing the approver "
										+ ex.toString());
					}
			...
			if (!autoApproveBank) {
				...
				if (toApproverEmail != null && toApproverEmail.trim().length() > 0) {
					...
					try {
						...
						logger.info(
								"Self Service Change Payroll Bank Info: An exception has occured with mailing the approver "
										+ ex.toString());
					}
