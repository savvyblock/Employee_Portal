...
public class AutoCompleteDao {
	...
	public List<Code> getEmployeeTempApproverSearch(String excludeEmpNumber, String search) {
	 ...
	 // Employee Portal Temporary Approver search with number
		public List<Code> getEmployeeTempApproverByNumber(String excludeEmpNumber, String search) {
			...
			sql.append("FROM bhr_emp_demo, bhr_emp_pay, sec_users ");
			sql.append("WHERE bhr_emp_demo.non_emp = 'N' AND bhr_emp_pay.emp_nbr != :excludeEmpNumber AND ");
			sql.append("(bhr_emp_pay.emp_nbr=bhr_emp_demo.emp_nbr and sec_users.emp_nbr=bhr_emp_demo.emp_nbr and sec_users.usr_deleted=0) and bhr_emp_demo.emp_nbr = :search  ");
		    if (lb_emp_nbr) {
				sql.append("order by code, descr ");
			} else {
				sql.append("order by descr, code ");
