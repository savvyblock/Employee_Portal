...
@Component
public class CreateEditLeaveRequestValidator implements Validator {
	...
	public boolean supports(Class<?> obj) {
		return LeaveRequestCommand.class.equals(obj);
	}

	@Override
	public void validate(Object object, Errors errors) {
		/* Validate on the command object, */
		if(object instanceof LeaveRequestCommand) {
			validateLeaveRequestCommand(object, errors);
		} /* Validate on array used in delete. */
		else {
			validateLeaveRequestArray(object, errors);
		}
	}
	
	public void validateLeaveRequestCommand(Object object, Errors errors) {
		LeaveRequestCommand command = (LeaveRequestCommand) object;
		
		List<LeaveType> leaveTypes = leaveRequestService.getLeaveTypes(command.getSelectedPayFrequencyCode(), command.getUserEmpNumber(), "");
		...
		savedRequests = leaveRequestService.getEmployeeLeaveRequestsPeriods(command.getUserEmpNumber());
		...
		for (LeaveRequest leaveRequest : command.getEditLeaveRequests()) {
			...
			if (!(leaveRequest.isEmpty() && leaveRequest.getId()==0)) {
				...
				String statusCode = leaveRequestService.getLeaveReqStatus(leaveRequest.getId());
				
				if(statusCode.equals("L")) {
					errors.reject("-1","Leave Requests are locked for processing.");
					break;
				}
				else if(statusCode.equals("C")) {
					errors.reject("-1","Leave Request has already been processed.");
					break;
				}
				else if(statusCode.equals("X")) {
					errors.reject("-1","Leave Request has already been deleted by your supervisor.");
					break;
				}
				...
				if (leaveRequest.getLeaveHoursDaily()==null || leaveRequest.getLeaveHoursDaily().compareTo(BigDecimal.ZERO)==0) {
					...
					if (leaveRequest.getLeaveRequested()==null || leaveRequest.getLeaveRequested().compareTo(BigDecimal.ZERO)==0) {
						...
						// get the leave info record corresponding to this request
		...
		//
		// for those that had no errors, check that the request date/time periods don't overlap with a previously defined one
	...
	public void validateLeaveRequestArray(Object object, Errors errors) {
		LeaveRequest[] leaveRequests = (LeaveRequest[]) object;
		
		for(LeaveRequest row : leaveRequests) {
			int id = row.getId();
			String statusCode = leaveRequestService.getLeaveReqStatus(id);
